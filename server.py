...
@app.route("/api/admin/commission/daily")
@require_admin
def api_admin_commission_daily():
    days = int(request.args.get("days", 30))
    return jsonify(db.commission_by_day(days=days))


@app.route("/api/admin/financial_report")
@require_admin
def api_admin_financial_report():
    start = request.args.get("start") or None
    end = request.args.get("end") or None
    return jsonify(db.financial_report(start=start, end=end))
...
@app.route("/api/admin/players/stats")
@require_admin
def api_admin_players_stats():
    return jsonify(db.player_counts())
...
def api_admin_game_history():
    ...
    bet = request.args.get("bet") or None
    start = request.args.get("start") or None
    end = request.args.get("end") or None
    return jsonify(db.get_games_history(limit=limit, bet=bet, start=start, end=end))
...
async def _game_event_handler(event, payload):
    ...
    if event == "win":
        ...
        for f in payload.get("flags", []):
            if f["type"] == "shared_device":
                detail = f"same device linked to {len(f['user_ids'])} accounts: {', '.join(f['user_ids'])}"
            else:
                detail = f"same IP linked to {len(f['user_ids'])} accounts: {', '.join(f['user_ids'])}"
            _notify_admin(
                f"🛡 Suspicious activity flagged!\n"
                f"Player: {payload.get('name', 'Player')} ({payload.get('user_id')})\n"
                f"Detail: {detail}\n\n"
                f"Review under Admin → Anti-cheat."
            )
        socketio.emit("admin_alert", {"type": "suspicious", "flags": payload.get("flags", [])}, room="admin_room")
