...
BOT_NAMES = ["Abebe", "Kebede", "Selam", "Meron", "Yared", "Liya", "Dawit", "Hana",
...
class GameRoom:
    ...
    def add_player(self, user_id, name):
        ...
        slot = {"user_id": uid, "name": name, "card": card, "marked": [], "won": False, "is_bot": False}
    ...
    def _maybe_add_bots(self):
        ...
        so a lone real player isn't left waiting indefinitely."""
    ...
    def _add_bot_slot(self, idx):
        ...
        self.players[uid] = {"user_id": uid, "name": name, "card": card, "marked": [],
                              "won": False, "is_bot": True}

    def add_bots_now(self, count):
        """Admin-triggered manual top-up, on top of whatever the auto-fill already added."""
        current_bots = sum(1 for s in self.players.values() if s.get("is_bot"))
        for i in range(count):
            self._add_bot_slot(current_bots + i + 1)
        if self.status == "waiting" and len(self.players) >= self.min_players:
            self._begin_countdown()
    ...
    def _start_game(self):
        ...
        total_pot = self.bet * real_player_count
        ...
        db.add_to_jackpot(self.bet, jackpot_amount)
        db.record_commission(self.game_id, self.bet, len(self.players), total_pot,
    ...
    def _finish(self, winners=None):
        ...
        if winners:
            ...
            for w in winners:
                ...
                db.credit_balance(w["user_id"], prize_each, f"Bingo win ({self.bet} ETB game - {pattern})")
                d = db.load()
                uid = w["user_id"]
                if uid in d["players"]:
                    ...
                    d["players"][uid]["wins"].append({
                        "amount": prize_each, "bet": self.bet, "pattern": pattern,
    ...
    def get_state(self, user_id=None):
        ...
        return {
            ...
            "winners": [{"name": w["name"], "pattern": self.winning_pattern_name(w)} for w in self.winners],
...
def join_lobby(user_id, name, bet, device_id=None, ip=None):
    ...
    if device_id or ip:
        db.register_device(user_id, device_id, ip)
...
# ---------------------------------------------------------------------------
# Admin controls over live games
# ---------------------------------------------------------------------------
