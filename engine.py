...
class GameRoom:
    ...
    def _finish(self, winners=None):
        ...
        # Persist a summary of this game for the admin Game History view.
        # This is separate from the ephemeral GameRoom object, which is
        # discarded once the lobby resets a few seconds from now.
        real_player_count = sum(1 for s in self.players.values() if not s.get("is_bot"))
        ...
        db.record_game_history({
            ...
            "bet": self.bet,
            "status": "finished" if winners else "no_winner",
            "real_players": real_player_count,
            "bots": bot_count,
            "numbers_drawn": len(self.drawn),
            "prize_per_winner": self.prize_per_winner,
            "commission": self.commission_amount,
            "winners": [{"name": w["name"], "user_id": w["user_id"],
                         "card_number": w.get("winning_card"),
                         "pattern": self.winning_pattern_name(w)} for w in (winners or [])],
            "started_at": datetime.fromtimestamp(self.start_time).strftime("%Y-%m-%d %H:%M") if self.start_time else "",
    ...
    def get_state(self, user_id=None):
        ...
        return {
            ...
            "winners": [{"name": w["name"], "pattern": self.winning_pattern_name(w),
                         "card_number": w.get("winning_card")} for w in self.winners],
            "my_cards": my_cards,
...
def enter_lobby(user_id, name, bet, device_id=None, ip=None):
    ...
    if device_id or ip:
        ...
        if new_flags:
            _fire_callbacks("suspicious", {"user_id": str(user_id), "name": name, "flags": new_flags})
